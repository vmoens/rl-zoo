...
results = []
for size, mass in (((0.28, 0.36, 0.1), 0.05),):
    for skills in ((1, 1),):
        for seed in range(1):
            ...
            try:
                ...
                with (
                    ...
                    for step in range(100):
                        chosen = (
                            (0, 0)
                            if float(base.get_state()["qpos"][0, -7]) > 0.68
                            else skills
                        )
                        transition = env.step(Forward(chosen)(td))
                ...
                env.close()
Path("pushing-stop-feasibility.json").write_text(
    json.dumps(
        dict(
